...
async function main() {
	...
	const eldoradoTacoId =
		restaurants.find((restaurant) => restaurant.name === "Eldorado Taco")
			?.id || 1;
	const laBartolaId =
		restaurants.find((restaurant) => restaurant.name === "La Bartola")
			?.id || 1;
	const elCatrinId =
		restaurants.find((restaurant) => restaurant.name === "El Catrin")?.id ||
		1;
	const mariachisId =
		restaurants.find((restaurant) => restaurant.name === "3 Mariachis")
			?.id || 1;
	...
	await prisma.item.createMany({
		data: [
			...
			},
			{
				name: "murgh salaad",
				description:
					"Chicken breast, mix greens, mint vinegar dressing",
				price: "$18.00",
			...
			},
			{
				...
				description:
					"Fresh homemade Gnocchi served in a blue cheese sauce",
				price: "$23.00",
				restaurant_id: stelvioId,
			},
			{
